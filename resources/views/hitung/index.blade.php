            ...
            <div class="box-content card white">
                ...
                <form action="{{ route('hitung.create') }}" method="post">
                    ...
                    <div class="card-content">
                        ...
                        <button type="submit" class=" btn btn-primary btn-sm waves-effect waves-light" name="ids" value="{{request()->segment(3) ? request()->segment(3) : NULL}}">Submit</button>
                        
                        @if(request()->segment(3))
                        <a href="{{ route('sub-hitung.reset',request()->segment(3)) }}" class="btn btn-warning btn-sm waves-effect waves-light">
                            Reset </a>
                        @else
                            ...
                            <tbody>
                                <tr class="text-center">
                                    ...
                                    @foreach ($data['kriteria'] as $kriteria)
                                        <td width="10%">{{ $kriteria['kode'] }} <small>({{ $kriteria['nama'] }})</small></td>
                                    @endforeach
                                </tr>
                                    ...
                                    @foreach ($data['kriteria'] as $kriteria2)
                                        <td>
                                            ...
                                            @else
                                                ...
                                                @foreach ($data['bobot'] as $items)
                                                    @if ($kriteria1['kode'] . $kriteria2['kode'] == $items['rel_1']['kode'] . $items['rel_2']['kode'])
                                                        {{ $items['nilai'] / $kriteria2['total_nilai'] }}
                                                        {{-- // Nilai sesuai dengan urutan pasangan kriteria --}}
                                                        @php
                                                            $nilai = $items['nilai']; // Simpan nilai bobot
                                                        @endphp
                                                    @endif
                                                    @if ($kriteria1['kode'] . $kriteria2['kode'] == $items['rel_2']['kode'] . $items['rel_1']['kode'])
                                                        {{ 1 / $items['nilai'] / $kriteria2['total_nilai'] }}
                                                        {{-- // Menggunakan nilai kebalikan --}}
                                                        @php
                                                            $nilai = 1 / $items['nilai']; // Simpan nilai kebalikan
                                                        @endphp
